...
async function getProjects(category?: string) {
  const d = db();
  const where = category && (G3_CATEGORIES as readonly string[]).includes(category)
...
export async function GET(request: Request) {
  try {
    ...
    // Enable CORS for g3ba
    const headers = new Headers();
    headers.set("Access-Control-Allow-Origin", "*");

    if (action === "projects") {
      const category = url.searchParams.get("category") || undefined;
      return NextResponse.json(await getProjects(category), { headers });
    }
    
    if (action === "projectBySlug") {
      const slug = url.searchParams.get("slug");
      if (!slug) return NextResponse.json({ error: "Missing slug" }, { status: 400, headers });
      return NextResponse.json(await getProjectBySlug(slug), { headers });
    }

    if (action === "services") return NextResponse.json(await getServices(), { headers });
    if (action === "team") return NextResponse.json(await getTeam(), { headers });
    if (action === "testimonials") return NextResponse.json(await getTestimonials(), { headers });
    
    if (action === "pageContent") {
      const slug = url.searchParams.get("slug");
      if (!slug) return NextResponse.json({ error: "Missing slug" }, { status: 400, headers });
      return NextResponse.json(await getPageContent(slug), { headers });
    }

    if (action === "stats") return NextResponse.json(await getStats(), { headers });

    return NextResponse.json({ error: "Invalid action" }, { status: 400, headers });
  } catch (error) {
    console.error("G3 public API error:", error);
